 ...
 * @param {Object} commands         Object containing all the commands available for the interpreter
 */
function executeCommander(command, commands, cb) {
    if (command[0] === 'help') {
        showHelp(commands);
        cb();
        ...
        if (command.length - 1 !== commands[command[0]].parameters.length) {
            ...
            if (commands[command[0]].callback == true){
               if (command.slice(1).length == 0) {
                  commands[command[0]].handler(function(err){
                     if(err) writer.log('Error in handled function');
                     cb();
                  });
               } else {
                  commands[command[0]].handler(command.slice(1), function(err){
                     if(err) writer.log('Error in handled function');
                     cb();
                  });
               }
...
function process_queue(commands, callback) {
   if (queue.length >= 1) {
      var groups = queue.pop();
      executeCommander(groups.map(removeQuotes) || [''], commands, function(){
         process_queue(commands, callback);
      });
   } else {
...
function initialize(commands, promptString) {
    ...
    rl.on('line', function (cmd) {
        ...
        if (groups) {
            ...
            if (!processing) {
               processing = true;
               process_queue(commands, function() {
                  processing = false;
                  prompt();
               });
            }
